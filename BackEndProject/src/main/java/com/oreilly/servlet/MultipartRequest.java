...
public class MultipartRequest {
   ...
   public MultipartRequest(HttpServletRequest request, String saveDirectory, int maxPostSize, String encoding, FileRenamePolicy policy) throws IOException {
      ...
      if (request == null) {
         ...
         if (!dir.isDirectory()) {
            ...
            if (request.getQueryString() != null) {//get method
               //Hashtable queryParameters = HttpUtils.parseQueryString(request.getQueryString());
               Hashtable queryParameters = parseQueryString(request.getQueryString());
   ...
   //add function
   private Hashtable parseQueryString(String requestQueryString) {
	   String[] queryParametersList = requestQueryString.split("&") ;
	   Hashtable queryParameters = new Hashtable();
	   for(int i=0; i < queryParametersList.length; i++) {
		   String[] keyValue = queryParametersList[i].split("=");
